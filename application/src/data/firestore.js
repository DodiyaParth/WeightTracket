...
export async function getProfiles(uids) {
  ...
  const out = {};
  snaps.forEach((s, i) => { if (s.exists()) out[unique[i]] = s.data(); });
  return out;
}
...
async function seriesEntriesFor(uid) {
  const snap = await getDocs(collection(db, 'users', uid, 'weights'));
...
async function fanOutSeries(uid) {
...
export async function updateWeight(uid, id, patch) {
...
export async function deleteWeight(uid, id) {
...
// ---- dashboards ---------------------------------------------------------
export async function listDashboards(uid) {
  const snap = await getDocs(query(collection(db, 'dashboards'), where('memberUids', 'array-contains', uid)));
...
export async function createDashboard(uid, { name, teamGoalLabel, teamGoalTarget }) {
  const data = {
...
async function safeRebuildPublic(dashboardId) {
...
export async function updateMemberRole(id, uid, role) {
...
export async function removeMember(id, uid) {
...
export async function getDashboardSeries(id) {
...
export async function setHabitMark(id, uid, habitId, date, value) {
...
// ---- NSV ----------------------------------------------------------------
export async function listNsv(id) {
...
export async function addNsv(id, uid, { date, text }) {
  await addDoc(collection(db, 'dashboards', id, 'nsv'), { uid, date, text, createdAt: Date.now() });
  await updateDoc(doc(db, 'dashboards', id), { updatedAt: Date.now() });
  changed();
}

export async function deleteNsv(dashboardId, noteId) {
...
export async function createInvite(dashboardId, { fromUid, fromName, toEmail, role }) {
  const d = await getDashboard(dashboardId);
  const email = (toEmail || '').toLowerCase().trim();
  const data = { dashboardId, dashboardName: d?.name || 'a dashboard', fromUid, fromName, fromInitial: initials(fromName), toEmail: email, role: role || 'editor', status: 'pending', createdAt: Date.now() };
...
export async function acceptInvite(inviteId, authUser) {
...
async function rebuildPublic(dashboardId) {
  const d = await getDashboard(dashboardId);
...
export async function setPublicLink(dashboardId, enabled) {
  const d = await getDashboard(dashboardId);
  if (enabled) {
...
export async function getPublicView(token) {
  ...
  return s.exists() && s.data().enabled ? s.data() : null;
}
