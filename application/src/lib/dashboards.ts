...
// A user's role on a dashboard, derived from its membership maps.
export function accessFor(dashboard, uid) {
  if (!dashboard) return 'viewer';
...
export const isEditable = (dashboard, uid) => ACCESS[accessFor(dashboard, uid)].editable;

const byUpdatedDesc = (a, b) => (b.updatedAt || 0) - (a.updatedAt || 0);

export const collaborating = (dashboards, uid) =>
  (dashboards || []).filter((d) => isEditable(d, uid)).sort(byUpdatedDesc);

export const viewOnly = (dashboards, uid) =>
  (dashboards || []).filter((d) => !isEditable(d, uid)).sort(byUpdatedDesc);

// Sidebar recents: collaboration first, then view-only, capped at `n` (§11.1).
export const recents = (dashboards, uid, n = 5) =>
...
export function landingRoute(dashboards, uid, now = Date.now()) {
  const within7 = (list) => list.filter((d) => now - (d.updatedAt || 0) <= 7 * DAY_MS);
...
function sortedMembers(dashboard) {
...
export function memberList(dashboard, profiles = {}) {
  return sortedMembers(dashboard).map((m, i) => {
    const p = profiles[m.uid] || m;
    return {
...
// color, not a name.
export function colorForMember(dashboard, uid) {
